...
const createPaySlip = async ( req, res ) => {
    try {
        ...
        if ( payment ) {
            ...
            const html = await ejs.renderFile( `${ __basedir }/views/paySlip.ejs`, data, { async : true } );
            // res.send(html)
...
const createAllPaySlips = async ( req, res ) => {
    ...
    async function renderToPDF( ps ) {
        ...
        // save pdf to file
        fs.writeFileSync( `${ __basedir }/public/pdfs/paySlip-${ ps.employee.EM_ID }-${ new Date().getTime() }.pdf`, pdfBuffer );
    }
}
